...
def run_deep_scraper():
    ...
    # Fallback to kolkataff.in
    print("Fetching from kolkataff.in fallback...")
    try:
        in_res = requests.get("https://kolkataff.in/", headers={'User-Agent': 'Mozilla/5.0'}, timeout=10)
        soup = BeautifulSoup(in_res.content, 'html.parser')
        tables = soup.find_all('table')
        for table in tables:
            rows = table.find_all('tr')
            ...
            if len(rows) >= 4 and '1234' in rows[1].get_text(strip=True).replace(' ', ''):
                patti_row = 2; single_row = 3
            else:
                patti_row = 1; single_row = 2
                
            if len(rows) <= single_row: continue
            ...
            for bazi_idx in range(num_bazis):
                p = pattis[bazi_idx]; s = singles[bazi_idx]
                if not p or not s or 'Tips' in p or 'Tips' in s or p == '' or s == '': continue
                p_val = re.sub(r'\D', '', p); s_val = re.sub(r'\D', '', s)
                if p_val and s_val:
                    res = p_val + s_val
                    all_records.append({'Date': date_parsed, 'Bazi': bazi_idx+1, 'Result_String': res, 'Patti': p_val, 'Single': s_val})
    except Exception as e:
        print(f"kolkataff.in fallback error: {e}")
    ...
    df['Has_Single'] = df['Single'].notna()
    df = df.sort_values(['Date', 'Bazi', 'Has_Single'])
    df = df.drop_duplicates(subset=['Date', 'Bazi'], keep='last').drop(columns=['Has_Single'])

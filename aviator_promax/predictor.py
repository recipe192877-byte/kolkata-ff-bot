...
class ProMaxPredictor:
    """
    Ensemble of 5 prediction algorithms:
      1. SMA  - Simple Moving Average crossover
      2. EMA  - Exponential Moving Average
      3. Streak Detector (hot/cold runs)
      4. Frequency Analyzer (last 30 rounds window)  ← FIX: was whole history
      5. Volatility Scorer
    """
    ...
    def save_history(self):
        try:
            with open(HISTORY_FILE, "w", encoding="utf-8") as f:
                json.dump({"history": list(self.history), "round_num": self.round_num}, f)
    ...
    def _streak_signal(self, h):
        ...
        recent = list(reversed(h[-10:]))   # last 10, newest first
        low_streak  = 0
        high_streak = 0
        for v in recent:
        ...
        # FIX: stronger thresholds for more reliable reversal signals
        if low_streak >= 5:
            return 3, f"Cold freeze: {low_streak} crashes (<2x) — strong reversal likely"
        elif low_streak >= 4:
            return 3, f"Low streak: {low_streak} consecutive reds — reversal likely"
        elif low_streak >= 3:
            return 2, f"Low streak: {low_streak} reds in a row"
        elif low_streak >= 2:
            return 1, f"Low streak: {low_streak} reds"
        elif high_streak >= 5:
            return -3, f"Hot streak: {high_streak} greens — crash very likely"
        elif high_streak >= 4:
    ...
    def _frequency_signal(self, h):
        # FIX: use only last 30 rounds for recency-weighted signal
        window = h[-30:] if len(h) >= 30 else h
        if len(window) < 10:
            return 0, "Not enough data for Frequency"
        n  = len(window)
        b1 = sum(1 for x in window if x < 1.5)       / n
        b2 = sum(1 for x in window if 1.5 <= x < 2)  / n
        b3 = sum(1 for x in window if 2 <= x < 5)    / n
        b4 = sum(1 for x in window if x >= 5)        / n
        crash_rate = b1 + b2
        if crash_rate > 0.7:
            return -1, f"High crash freq (last 30): {crash_rate*100:.0f}% under 2x"
        elif crash_rate < 0.4:
            return 2, f"Low crash freq (last 30): {crash_rate*100:.0f}% under 2x"
        elif b4 > 0.2:
            return 1, f"High 5x+ freq (last 30): {b4*100:.0f}%"
    ...
    def predict(self):
        ...
        # FIX: threshold now says 3 (the actual minimum), not misleading "10"
        if n < 3:
            return {
                "action":     "WATCH",
                "cashout":    "-",
                ...
                "reason":     f"Need more data ({n}/3 rounds collected)",
                "signal":     "neutral",
                "votes":      {},
                "stats":      self._stats(h),
                "total_score": 0,
        ...
        if total >= 4:
            ...
            action, cashout, signal = "BET SAFE 🛡️", "1.50x", "safe"
            conf_label = "Weak Buy"
        elif total <= -4:
        ...
        priority_keys = ["streak", "extreme", "abnormal", "bullish", "bearish",
                         "crash", "freeze", "cold", "hot"]

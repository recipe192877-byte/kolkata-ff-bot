...
class ProMaxPredictor:
    ...
    2. EMA  - Exponential Moving Average  (fixed: no double-counting)
    3. Streak Detector
    ...
    def load_history(self):
        ...
        try:
            ...
            print(f"[PREDICTOR] Loaded {len(saved)} rounds from cache (Round #{self.round_num})")
        except Exception as e:
    ...
    def _sma_signal(self, h):
        ...
        if sma5 > sma20 * 1.1:
            return 2, f"SMA bullish: {sma5:.2f} > {sma20:.2f}"
        elif sma5 < sma20 * 0.9:
            return -2, f"SMA bearish: {sma5:.2f} < {sma20:.2f}"
        return 0, f"SMA neutral: {sma5:.2f} vs {sma20:.2f}"

    def _ema_signal(self, h):
        """Fixed EMA: warm-up starts from oldest value, no double-count."""
        if len(h) < 3:
            return 0, "Not enough data"
        k = 2 / (min(len(h), 10) + 1)
        ema = h[0]  # seed with oldest
        for v in h[1:]:  # iterate from second onwards (fixed)
            ema = v * k + ema * (1 - k)
        last = h[-1]
        if last > ema * 1.15:
    ...
    def _streak_signal(self, h):
        ...
        for v in recent:
            if v >= 2.0:
                ...
                break
        if low_streak >= 4:
            return 3, f"Low streak: {low_streak} reds in a row"
        elif low_streak >= 3:
            return 2, f"Low streak: {low_streak} reds"
        elif low_streak >= 2:
            return 1, f"Low streak: {low_streak} reds"
        elif high_streak >= 4:
            return -2, f"High streak: {high_streak} greens (crash due)"
        elif high_streak >= 3:
    ...
    def _frequency_signal(self, h):
        ...
        if crash_rate > 0.7:
            ...
            return 1, f"High freq of 5x+: {b4*100:.0f}%"
        return 0, f"Normal distribution: crash={crash_rate*100:.0f}%"

    def _volatility_signal(self, h):
        if len(h) < 5:
            return 0, "Not enough data"
        std = statistics.stdev(h[-10:]) if len(h) >= 10 else statistics.stdev(h)
        avg = statistics.mean(h[-10:]) if len(h) >= 10 else statistics.mean(h)
        last = h[-1]
        if last > avg + 2 * std:
            return -3, f"Extreme outlier high: {last:.2f}x (crash likely)"
        elif last < avg - std and last < 1.5:
            return 2, f"Abnormally low: {last:.2f}x (bounce possible)"
    ...
    def predict(self):
        ...
        reasons = [r3, r5, r1, r2, r4]
        reason = next((r for r in reasons if any(k in r.lower() for k in
                       ["streak","extreme","bullish","bearish","abnormal"])), reasons[0])
    ...
    def _stats(self, h):
        n = len(h)
        if n == 0:
            return {"total": 0, "avg_all": 0, "avg_10": 0, "avg_5": 0,
        ...
        return {
            ...
            "crash_rate": round(sum(1 for x in recent10 if x < 2.0) / len(recent10) * 100, 1),
